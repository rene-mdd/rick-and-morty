      ...
      {data ? (
        <Card centered>
          ...
          <Card.Content>
            ...
            <Card.Meta>
              <p>Status <span style={{ color: data.character.status !== 'Alive' ? '#FE2C54' : '#0CB577' }}>{data.character.status}</span></p>
            </Card.Meta>
            <Divider />
            <Card.Description>
              <p>Location: <span>{data.character.location.name || 'No data'}</span></p>
              <p>Origin: <span>{data.character.origin.name}</span></p>
              <p>Gender: <span>{data.character.gender}</span></p>
              <p>Origin: <span>{data.character.origin.name}</span></p>
            </Card.Description>
          </Card.Content>
          <Card.Content extra>
            <p>Species <span style={{ color: '#CFAF7B' }}>{data.character.species}</span></p>
          </Card.Content>
        </Card>
      ) : <h1>Loading</h1>}
      <Container>
